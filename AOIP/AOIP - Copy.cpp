...
void SAPListener()
{
	...
	//	bool bFound = false;
		...
		{
			if (!strcmp(Stream.DevName, TransmissionStreams[i].DevName))
				goto END;
		}
...
static inline float ToFloat(int Sample)
{
	...
	return (float)(((double)Sample) / 8388608.f);
...
void MainLoop()
{
	...
	for (unsigned char i = 0; i < Index; ++i)
	{
		//if (!strncmp(Streams[i].DevName, "TIO2", 4))
	...
	double TSCAbsInterval = 0.001;// 1.0000000000000 / (double)IntervalFreq;//(double)TSCFreq / (double)IntervalFreq;	//Number of ticks per interval
	printf("TSCFreq %f, Packet time %fs\n", TSFreq, TSCAbsInterval);
	
	//Timer absolute values.
	unsigned long long Start;// Moved to global scope
...
#define SEQOFF 72*/
	...
	double ld;
	unsigned int Seq = 0;
	while (Start == 0) //We can't check for a litteral zero since it's not a binary number.
	{
		...
		if (GetDstIP((const char*)Data) == ( (129 << 24) | (1 << 16) | (0 << 8) | 224) && Header->len == 86)
		{
			...
			if (ntohs(GetDstPort((const char*)Data)) == 319)
			{
				...
				*((unsigned long long*)PTPMasterClock) = *((unsigned long long*)(Data + 20 + UDPPAYLOAD));
				LocalTS = (Header->ts.tv_usec * 1000) + (Header->ts.tv_sec * 1000000000);	//We need to calculate local delay
			}
			//Follow up
			else if (ntohs(GetDstPort((const char*)Data)) == 320 && LocalTS)
			{
				//Get follow up msg
				//Seconds
				Start = ntohl(*((unsigned int*)(Data + 36 + UDPPAYLOAD)));

				//Convert to NanoSeconds
				Start *= 1000000000;

				//NanoSeconds
				Start += ntohl((*(unsigned int*)(Data + 40 + UDPPAYLOAD)));

				//Calculate follow up delay
				Start += ((Header->ts.tv_usec * 1000) + (Header->ts.tv_sec * 1000000000) - LocalTS);
				Stop = Start + 1000000;
				LocalDelta = GetTS();
				LocalTS = 0;
			}
		...
		while (Start < Stop) //Frame loop 1ms typically
		{
			if (ReadIndex != GStepperIndex)	//Make sure we have data to process
			{
				...
				//	LocalDelta = TempDelta;
					...
					{
						TransmissionStreams[i].Timestamp = ((unsigned int)(((Start) * 48000.0f)));
					}
			...
			if (Status == 1)
			{
				if (false)//Header->len > 300 && (Stream = GetStream(GetDstIP((const char*)Data))) != nullptr)
				{
					if (Stream->bFlags & S_WAIT_OPEN)
					{
						//Calculate number of packets per second, and mark stream as ready.
						Stream->PackSamples = (((Header->len - PAYLOADOFFSET) / Stream->Channels /* Sample chunk for one channel*/) / 3 ); // (Stream->ByteDepth));
					...
					else if (Stream->bFlags & S_OPEN)
					{
						...
						if (Stream->Timestamp + Stream->PackSamples < GetTimestamp((char*)Data))
						{
							...
							Stream->SampleIndex &= 0x7F;
							printf("Late packet %i\n", GetTimestamp((char*)Data) - (Stream->Timestamp + Stream->PackSamples));
						}
				...
				else if (Header->len == 86)
				{
					...
					{
						//Update PTP Master address
						LocalTS = (((double)Header->ts.tv_usec / 1000000.0) + (double)Header->ts.tv_sec);
						//grr += "Got packet\n";
						//*((unsigned long long*)PTPMasterClock) = *((unsigned long long*)(Data + 20 + UDPPAYLOAD));
					...
					//Follow up
					else if (ntohs(GetDstPort((const char*)Data)) == 320 && LocalTS > 0)
					{
						...
						double CurTS = (((double)Header->ts.tv_usec / 1000000.0) + (double)Header->ts.tv_sec) - LocalTS;
						//CurTS /= TSFreq;

						//if ( fabs((CurTS + TS) - Start) < 0.0003)
						{
							LocalTS = 0;
							LocalDelta = GetTS();
							Start = CurTS + TS;
							if (Start > (Stop))
								Start = (Stop);
							else if (Start < (Stop - TSCAbsInterval))
								Start = (Stop - TSCAbsInterval);
						...
						/*for (unsigned char i = 0; i < NumTStreams; ++i)
						{
							if (abs((long)(TransmissionStreams[i].Timestamp - ((unsigned int)(((Start) * 48000.0))))) > 48)	//Only do occasional corrections
								TransmissionStreams[i].Timestamp = ((unsigned int)(((Start) * 48000.0)));
						} */
