...
//	unsigned long arg = 1;
	...
	while (k < 10000)	//Clean out the buffer
	{
		//pcap_next_ex(listener, &Header, &Data);
		recv(PTPSOCK, (char*)&Data, 2048, 0);
		recv(PTPSlave, (char*)&Data, 2048, 0);
		++k;
	}
	...
	while (Start == 0.0) //We can't check for a litteral zero since it's not a binary number.
	{
		...
		Bytes = recvfrom(PTPSOCK, (char*)&Data, 2048, 0, (sockaddr*)&binder, &Sz);
		SockState = R_PTPF;
				...
				if (Bytes > 0)
				{
					...
					//if ((Stream = GetStream(binder.sin_addr.S_un.S_addr, binder.sin_port)) != nullptr)
					{
						...
						else if (Stream->bFlags & S_OPEN)
						{
							...
							{
								Stream->SampleIndex += GetTimestamp(Data) - (Stream->Timestamp + 48);
								Stream->SampleIndex &= 0x7F;
								union
								{
									unsigned int IP;
							...
							else if (Stream->Timestamp + 48 > GetTimestamp(Data))
							{
